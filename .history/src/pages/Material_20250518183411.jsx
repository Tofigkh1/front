...
const handleViewLogs = async (id, name) => {
  try {
    const token = localStorage.getItem("token");
    ...
    setSelectedLogs(response.data.data); // log'lar bu şekilde geliyorsa
        ...
        <ul className="space-y-2">
          {selectedLogs.map((log, index) => (
            <li key={index} className="border p-3 rounded bg-gray-100">
              <div><strong>Tarix:</strong> {new Date(log.created_at).toLocaleString()}</div>
              <div><strong>Əməliyyat:</strong> {log.action}</div>
              <div><strong>Miqdar:</strong> {log.quantity}</div>
              {log.text && <div><strong>Qeyd:</strong> {log.text}</div>}
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-gray-500">Log yoxdur.</p>
      )}
