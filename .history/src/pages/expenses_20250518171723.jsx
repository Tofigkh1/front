...
// ✅ Kateqoriyaları yüklə və ilkini seç
const fetchCategories = async () => {
  try {
    ...
    if (Array.isArray(list)) {
      setCategories(list);
      if (list.length > 0) {
        setSelectedCategory(list[0].id); // avtomatik ilk kateqoriyanı seç
      }
    }
  } catch (error) {
    handleError(error);
...
useEffect(() => {
  ...
  if (!amount.trim() || !reason.trim() || !selectedCategory) return;
