...
const handleCustomModal = (item) => {
  ...
  if (item.stocks) {
    // Gelen veri bir SET → ilk stock'u al
    selectedStock = item.stocks[0]; // veya başka bir seçim
  } else {
    // Gelen veri direkt stock
    selectedStock = stocks.find((stock) => stock.id === item.id);
  }
  ...
  if (selectedStock) {
    ...
    console.warn("Stock not found:", item);
  }
};
...
onClick={() => {
  if (set.stocks && set.stocks.length > 0) {
    handleCustomModal(set.stocks[0]); // veya başka bir seçim mantığı
  } else {
    console.warn("Set has no stocks");
  }
}}
