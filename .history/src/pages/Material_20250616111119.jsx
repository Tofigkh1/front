...
const material = rawMaterials.find((m) => m.name === selectedMaterialName);
...
{selectedLogs.length > 0 ? (
  selectedLogs.map((log) => {
    // rawMaterials'dan ilgili unit değerini al
    const material = rawMaterials.find((m) => m.id === selectedMaterialId);
    const unitLabel = category.find((cat) => cat.id === material?.unit)?.label || "vahid";
...
<div className="sm:hidden space-y-4">
  ...
  {rawMaterials.map((item, index) => (
    ...
    >
      ...
      <div   onClick={() => handleViewLogs(item.id)}>
             <div>
