        ...
        {editId === item.id ? (
          ...
          <button
            onClick={() =>
              setEditId(item.id) ||
              setEditedValues({
                quantity: item.stock?.quantity ?? "", // yalnız quantity redaktə olunacaq
              })
            }
        ...
        <button
          onClick={async () => {
            try {
              ...
              await axios.post(`${base_url}/raw-materials/${selectedItemId}/increase`, payload, {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
              });

              toast.success("Məhsul artırıldı", { autoClose: 1000 });
              setShowModal(false);
              setExplanationText("");
              setSelectedItemId(null);
              fetchData();
            } catch (err) {
              console.error("Artırılarkən xəta:", err);
              toast.error("Xəta baş verdi. Yenidən yoxlayın.", { autoClose: 1000 });
            }
          }}
