...
const fetchStockSets = async () => {
  ...
  const handleConfirmOrder = async (items) => {
  try {
    const orderData = {
      stocks: items.map(item => ({
        stock_id: item.id,
        quantity: item.quantity, // Her ürünün miktarı
    ...
    dispatch(clearBasket()); // Redux'tan sepeti temizle
    setOpenClick(false);
    setOpenClickModal(false);
  } catch (error) {
    if (error.response?.status === 400) setQrcodedontOrder(true);
