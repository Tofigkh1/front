...
const handleCustomModal = (item) => {
  ...
  if (item.stocks && Array.isArray(item.stocks)) {
    // Gelen veri bir set → içindeki ilk stock'u kullan
    if (item.stocks.length === 0) {
      console.warn("Stock set has no stocks");
      return;
    }
    selectedStock = item.stocks[0];
  } else {
    // Gelen veri doğrudan bir stock
