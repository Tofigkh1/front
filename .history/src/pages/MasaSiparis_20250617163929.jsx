...
import PasswordScreen from "../components/ScreenPassword2";
...
<td
  onClick={(e) => {
    e.stopPropagation();
    setPendingDeleteItem({ id: item.id, pivot_id: item?.pivot_id, quantity: item.quantity });
...
>
  ...
  onClick={() => {
    setPendingDeleteItem({ id: item.id, pivot_id: item?.pivot_id, quantity: item.quantity });
    setIsPasswordModalOpen(true); // önce modal açılsın
  }}
  className="text-red-500 cursor-pointer text-center"
>
  <i className="fa-solid fa-trash-can"></i>
...
{isPasswordModalOpen && (
  <PasswordScreen
    onSuccess={() => {
      // burada silme isteği gönder
  handleRemoveStock(pendingDeleteItem.id, pendingDeleteItem.pivot_id, pendingDeleteItem.quantity);
