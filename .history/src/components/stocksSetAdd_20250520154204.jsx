...
formData.additionalPrices.forEach((priceObj, index) => {
  if (priceObj.price && priceObj.unit && priceObj.count) {
    formDataToSend.append(`additional_prices[${index}][price]`, priceObj.price);
    formDataToSend.append(`additional_prices[${index}][unit]`, priceObj.unit);
    formDataToSend.append(`additional_prices[${index}][count]`, priceObj.count);
  }
});
  
    try {
      const token = localStorage.getItem("token");
      const stockResponse = await axios.post(`${base_url}/stock-sets`, formDataToSend, {
        ...getAuthHeaders(),
        headers: {
    ...
    if (stockId && selectedRawMaterials.length > 0) {
      await axios.post(
        ...
        {
       formDataToSend.append('raw_materials', JSON.stringify(
  selectedRawMaterials.map(mat => ({
    raw_material_id: parseInt(mat.id),
