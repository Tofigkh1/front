...
.then(res => {
  ...
  // Formda görüntülemek için selectedStocks'u da ayarla
...
const handleSave = async () => {
  ...
  // Burada selectedStocks state'ini kullanıyoruz
  const validStocks = selectedStocks.map((stock) => ({
    id: Number(stock.id),
    quantity: Math.max(Number(stock.quantity || 0), 1), // minimum 1
  ...
  try {
    const payload = {
      ...
      stocks: validStocks, // doğru state burada
    };
    ...
    <div key={index} className="flex gap-2 mb-2 items-center">
      ...
      <select
        ...
        onChange={(e) => {
          ...
          const selectedItem = stockSets.find((item) => item.id === parseInt(selectedId));
