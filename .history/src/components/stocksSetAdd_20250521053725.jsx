...
const handleSubmit = async (e) => {
  ...
  try {
    const response = await axios.post(`${base_url}/stock-sets`, payload, {
      headers: getAuthHeaders() // JSON için doğru header'ları ekle
    });
    setAddStok(false);
  } catch (error) {
 ...
 <h3 className="mb-2">Stoklar</h3>
  {selectedStocks.map((stock, index) => (
    <div key={index} className="flex gap-2 mb-2">
      ...
      <input
        type="number"
        className="border rounded py-2 px-3 w-1/4 text-sm font-medium"
